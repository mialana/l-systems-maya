...
// constructor
MAT_TEMPLATE inline matrixT::matrix(size_t row, size_t col)
{
    _m = new base_mat(row, col, 0);
}

// copy constructor
MAT_TEMPLATE inline matrixT::matrix(const matrixT& m)
{
    _m = m._m;
    _m->Refcnt++;
}
...
MAT_TEMPLATE inline void matrixT::clone()
{
    _m->Refcnt--;
...
// destructor
MAT_TEMPLATE inline matrixT::~matrix()
{
    if (--_m->Refcnt == 0)
    {
        delete _m;
...
// assignment operator
MAT_TEMPLATE inline matrixT& matrixT::operator=(const matrixT& m) _NO_THROW
{
    m._m->Refcnt++;
    if (--_m->Refcnt == 0)
    {
...
//  reallocation method
MAT_TEMPLATE inline void matrixT::realloc(size_t row, size_t col)
{
...
MAT_TEMPLATE inline void matrixT::SetSize(size_t row, size_t col) _NO_THROW
{
    size_t i, j;
...
MAT_TEMPLATE inline T& matrixT::operator()(size_t row, size_t col) _THROW_MATRIX_ERROR
{
...
// subscript operator to get/set individual elements
MAT_TEMPLATE inline T matrixT::operator()(size_t row, size_t col) const _THROW_MATRIX_ERROR
{
    if (row >= _m->Row || col >= _m->Col)
    {
        REPORT_ERROR("matrixT::operator(): Index out of range!");
...
MAT_TEMPLATE inline istream& operator>>(istream& istrm, matrixT& m)
{
...
MAT_TEMPLATE inline ostream& operator<<(ostream& ostrm, const matrixT& m)
{
    for (size_t i = 0; i < m.RowNo(); i++)
    {
        for (size_t j = 0; j < m.ColNo(); j++)
...
MAT_TEMPLATE inline bool operator==(const matrixT& m1, const matrixT& m2) _NO_THROW
{
...
MAT_TEMPLATE inline matrixT& matrixT::operator+=(const matrixT& m) _THROW_MATRIX_ERROR
{
...
MAT_TEMPLATE inline matrixT& matrixT::operator*=(const T& c) _NO_THROW
{
    if (_m->Refcnt > 1)
    {
        clone();
...
MAT_TEMPLATE inline matrixT& matrixT::operator^=(const size_t& pow) _THROW_MATRIX_ERROR
{
    matrixT temp(*this);
...
MAT_TEMPLATE inline matrixT matrixT::operator-() _NO_THROW
{
...
MAT_TEMPLATE inline matrixT operator+(const matrixT& m1, const matrixT& m2) _THROW_MATRIX_ERROR
{
    matrixT temp = m1;
    temp += m2;
    return temp;
...
MAT_TEMPLATE inline matrixT operator*(const matrixT& m, const T& no) _NO_THROW
{
    matrixT temp = m;
    temp *= no;
    return temp;
...
// binary scalar multiplication operator
MAT_TEMPLATE inline matrixT operator*(const T& no, const matrixT& m) _NO_THROW
{
    return (m * no);
}
...
// binary scalar division operator
MAT_TEMPLATE inline matrixT operator/(const T& no, const matrixT& m) _THROW_MATRIX_ERROR
{
    return (!m * no);
}
...
MAT_TEMPLATE inline matrixT operator^(const matrixT& m, const size_t& pow) _THROW_MATRIX_ERROR
{
    matrixT temp = m;
    temp ^= pow;
    return temp;
...
MAT_TEMPLATE inline matrixT operator~(const matrixT& m) _NO_THROW
{
...
MAT_TEMPLATE inline matrixT operator!(const matrixT m) _THROW_MATRIX_ERROR
{
    matrixT temp = m;
    return temp.Inv();
}
...
// inversion function
MAT_TEMPLATE inline matrixT matrixT::Inv() _THROW_MATRIX_ERROR
{
    size_t i, j, k;
    T a1, a2, *rowptr;
...
MAT_TEMPLATE inline matrixT matrixT::Solve(const matrixT& v) const _THROW_MATRIX_ERROR
{
    size_t i, j, k;
    T a1;
...
MAT_TEMPLATE inline void matrixT::Null(const size_t& row, const size_t& col) _NO_THROW
{
    if (row != _m->Row || col != _m->Col)
    {
        realloc(row, col);
...
// set zero to all elements of this matrix
MAT_TEMPLATE inline void matrixT::Null() _NO_THROW
{
    if (_m->Refcnt > 1)
    {
        clone();
...
MAT_TEMPLATE inline void matrixT::Unit(const size_t& row) _NO_THROW
{
...
// set this matrix to unity
MAT_TEMPLATE inline void matrixT::Unit() _NO_THROW
{
    if (_m->Refcnt > 1)
    {
        clone();
...
MAT_TEMPLATE inline int matrixT::pivot(size_t row)
{
    int k = int(row);
    double amax, temp;
...
MAT_TEMPLATE inline T matrixT::Det() const _THROW_MATRIX_ERROR
{
    size_t i, j, k;
    T piv, detVal = T(1);
...
MAT_TEMPLATE inline T matrixT::Norm() _NO_THROW
{
    T retVal = T(0);
...
MAT_TEMPLATE inline T matrixT::Cond() _NO_THROW
{
    matrixT inv = !(*this);
...
MAT_TEMPLATE inline T matrixT::Cofact(size_t row, size_t col) _THROW_MATRIX_ERROR
{
    size_t i, i1, j, j1;
...
MAT_TEMPLATE inline matrixT matrixT::Transpose() _THROW_MATRIX_ERROR
{
    /*
       matrixT temp(_m->Col,_m->Row);

       for (size_t i=0; i < this->_m->Row; i++)
          for (size_t j=0; j < this->_m->Col; j++)
              temp._m->Val[j][i] = this->_m->Val[i][j];
    */
    return ~(*this);
}

// calculate adjoin of a matrix
MAT_TEMPLATE inline matrixT matrixT::Adj() _THROW_MATRIX_ERROR
{
    if (_m->Row != _m->Col)
    {
...
MAT_TEMPLATE inline bool matrixT::IsSingular() _NO_THROW
{
    if (_m->Row != _m->Col)
    {
        return false;
...
MAT_TEMPLATE inline bool matrixT::IsDiagonal() _NO_THROW
{
    if (_m->Row != _m->Col)
    {
        return false;
...
MAT_TEMPLATE inline bool matrixT::IsScalar() _NO_THROW
{
    if (!IsDiagonal())
    {
        return false;
...
MAT_TEMPLATE inline bool matrixT::IsUnit() _NO_THROW
{
...
MAT_TEMPLATE inline bool matrixT::IsNull() _NO_THROW
{
    for (size_t i = 0; i < _m->Row; i++)
    {
        for (size_t j = 0; j < _m->Col; j++)
...
MAT_TEMPLATE inline bool matrixT::IsSymmetric() _NO_THROW
{
    if (_m->Row != _m->Col)
    {
        return false;
...
MAT_TEMPLATE inline bool matrixT::IsSkewSymmetric() _NO_THROW
{
    if (_m->Row != _m->Col)
    {
        return false;
...
MAT_TEMPLATE inline bool matrixT::IsUpperTriangular() _NO_THROW
{
    if (_m->Row != _m->Col)
    {
        return false;
...
MAT_TEMPLATE inline bool matrixT::IsLowerTriangular() _NO_THROW
{
    if (_m->Row != _m->Col)
    {
        return false;
...
MAT_TEMPLATE inline TranslationMatrix<T>::TranslationMatrix(T x, T y, T z) : matrixT(4, 4)
{
    this->_m->Val[0][0] = T(1);
    this->_m->Val[0][1] = T(0);
    this->_m->Val[0][2] = T(0);
...
MAT_TEMPLATE inline RotationMatrix<T>::RotationMatrix(int axis, double angle) : matrixT(4, 4)
{
    if (axis == 0)  // X
    {
        this->_m->Val[0][0] = 1;
        ...
        this->_m->Val[0][3] = 0;
        this->_m->Val[1][0] = 0;
        this->_m->Val[1][1] = cos(angle);
        this->_m->Val[1][2] = -sin(angle);
        this->_m->Val[1][3] = 0;
        this->_m->Val[2][0] = 0;
        this->_m->Val[2][1] = sin(angle);
        this->_m->Val[2][2] = cos(angle);
        this->_m->Val[2][3] = 0;
        this->_m->Val[3][0] = 0;
        this->_m->Val[3][1] = 0;
        this->_m->Val[3][2] = 0;
    ...
    {
        this->_m->Val[0][0] = cos(angle);
        this->_m->Val[0][1] = 0;
        this->_m->Val[0][2] = sin(angle);
        this->_m->Val[0][3] = 0;
        this->_m->Val[1][0] = 0;
        this->_m->Val[1][1] = 1;
        this->_m->Val[1][2] = 0;
        this->_m->Val[1][3] = 0;
        this->_m->Val[2][0] = -sin(angle);
        this->_m->Val[2][1] = 0;
        this->_m->Val[2][2] = cos(angle);
        this->_m->Val[2][3] = 0;
        this->_m->Val[3][0] = 0;
    ...
    {
        this->_m->Val[0][0] = cos(angle);
        this->_m->Val[0][1] = -sin(angle);
        this->_m->Val[0][2] = 0;
        this->_m->Val[0][3] = 0;
        this->_m->Val[1][0] = sin(angle);
        this->_m->Val[1][1] = cos(angle);
        this->_m->Val[1][2] = 0;
        this->_m->Val[1][3] = 0;
        this->_m->Val[2][0] = 0;
...
MAT_TEMPLATE inline RotationMatrix<T>::RotationMatrix(const vec3& c1, const vec3& c2, const vec3& c3)
    : matrixT(4, 4)
{
    this->_m->Val[0][0] = c1[0];
    this->_m->Val[0][1] = c2[0];
    this->_m->Val[0][2] = c3[0];
    this->_m->Val[0][3] = 0;
    this->_m->Val[1][0] = c1[1];
    this->_m->Val[1][1] = c2[1];
    this->_m->Val[1][2] = c3[1];
    this->_m->Val[1][3] = 0;
    this->_m->Val[2][0] = c1[2];
    this->_m->Val[2][1] = c2[2];
    this->_m->Val[2][2] = c3[2];
    this->_m->Val[2][3] = 0;
    this->_m->Val[3][0] = 0;
    this->_m->Val[3][1] = 0;
    this->_m->Val[3][2] = 0;
...
MAT_TEMPLATE inline ScaleMatrix<T>::ScaleMatrix(T x, T y, T z) : matrixT(4, 4)
{
    this->_m->Val[0][0] = x;
    this->_m->Val[0][1] = T(0);
    this->_m->Val[0][2] = T(0);
...
MAT_TEMPLATE inline vec3 operator*(const matrixT& m,
                                   const vec3& v) _THROW_MATRIX_ERROR  // transform point
{
    matrixT vMat(4, 1);
    vMat(0, 0) = v[0];
    vMat(1, 0) = v[1];
...
MAT_TEMPLATE inline vec3 operator^(const matrixT& m,
                                   const vec3& v) _THROW_MATRIX_ERROR  // transform vector
{
    matrixT vMat(4, 1);
    vMat(0, 0) = v[0];
    vMat(1, 0) = v[1];
